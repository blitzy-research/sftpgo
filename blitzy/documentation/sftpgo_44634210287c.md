...
### 5.6 Child Process Environment Inheritance

**Source: `sftpd/ssh_cmd.go:324`, `sftpd/sftpd.go:421–429`**

In `getSystemCommand()` at `sftpd/ssh_cmd.go:324`, the child process is created via:

```go
cmd := exec.Command(c.command, args...)                              // line 324
```

No subsequent assignment to `cmd.Env` exists anywhere in `getSystemCommand()` or `executeSystemCommand()`. In Go's `os/exec` package, when `cmd.Env` is `nil`, the child process **inherits the parent process's complete environment**. This means every environment variable available to the SFTPGo server process — potentially including database credentials, API tokens, cloud provider secrets, or other sensitive configuration — is passed to the child process (e.g., `rsync`, `git-receive-pack`).

**Contrast with notification commands:** The author was clearly aware of environment control in other code paths. `executeNotificationCommand()` at `sftpd/sftpd.go:421–429` explicitly sets the child environment:

```go
cmd.Env = append(os.Environ(),                                      // line 422
    fmt.Sprintf("SFTPGO_ACTION=%v", operation),
    // … additional SFTPGO_ACTION_* variables …
)
```

This explicit `cmd.Env` assignment in the notification path contrasts with the SSH exec path, where no such assignment exists.

**Mitigating factors:**

1. **UID/GID credential delegation** (`cmd_unix.go:10–16`): The child process runs under the user's configured UID/GID, not SFTPGo's service account. This limits what the child process can do with any leaked credentials, since OS-level permissions still apply.
2. **No SSH "env" request handler** (`server.go:318–330`): The SSH channel handler only processes `"subsystem"` and `"exec"` request types. There is no handler for the SSH `"env"` request type, so SSH clients **cannot inject arbitrary environment variables** into the session. The inherited environment comes solely from the SFTPGo server process.
3. **Restricted command set**: The child processes are well-known utilities (`rsync`, `git-receive-pack`, `git-upload-pack`, `git-upload-archive`, `scp`) — not arbitrary binaries. These tools do not typically exfiltrate environment variables, though a vulnerability in any of them could theoretically expose environment contents.

**Risk assessment:** This is a **defense-in-depth gap**, not an immediately exploitable vulnerability. Exploitation would require either a vulnerability in one of the allowed child binaries that leaks environment contents, or a misconfiguration that grants an attacker interactive access to the child process's environment. Nonetheless, setting `cmd.Env` to a minimal, scrubbed environment in `getSystemCommand()` would reduce the attack surface.
