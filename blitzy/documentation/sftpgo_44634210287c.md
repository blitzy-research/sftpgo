...
- **Line 166** — two distinct paths depending on `isNewFile`:
  - **New file** (`isNewFile=true`): `numFiles` started at 1, decremented to 0. With `numFiles == 0` and `bytesReceived == 0`, the condition `numFiles != 0 || t.bytesReceived > 0` is **FALSE** → `UpdateUserQuota()` is **NOT called** → quota is clean.
  - **Overwrite** (`isNewFile=false`): `numFiles` started at 0, decremented to -1. With `numFiles == -1` and `bytesReceived == 0`, the condition `-1 != 0 || 0 > 0` is **TRUE** → `UpdateUserQuota(user, -1, 0, false)` **IS called**, decrementing the user's file count by 1. This correctly reflects that the original file was destroyed (see below).
- **Artifact — new file**: Temp file deleted; target file was never created. No data loss.
- **Artifact — overwrite**: Temp file deleted, **but the original file at the target path is irreversibly lost**. This is because `handleSFTPUploadToExistingFile()` renamed the original file FROM the target path TO the atomic temp path (`c.fs.Rename(requestPath, filePath)` at `sftpd/handler.go`, line 468), then `c.fs.Create(filePath, osFlags)` at line 476 truncated that temp path. On error, `os.Remove()` at `transfer.go` line 140 deleted the temp file. The target path is now empty — both old content and new upload data are destroyed.
- **Quota — new file**: No change — correctly reversed.
- **Quota — overwrite**: File count decremented by 1 via `UpdateUserQuota(-1, 0)`. Additionally, the old file's size was already deducted at upload initiation via `UpdateUserQuota(0, -fileSize, false)` at `sftpd/handler.go`, line 486. The combined effect is file count reduced by 1 and used size reduced by the old file's size, which correctly reflects the on-disk state (both files gone).
...
| Atomic mode (1) — error, new file | **No** (deleted) | No | N/A | Temp deleted; target was never created |
| Atomic mode (1) — error, overwrite | **No** (deleted) | **No** | **N/A — old content lost** | Old file was renamed to temp path (`handler.go` L468), then `Create()` truncated temp (`osfs.go` L64–73), then temp deleted (`transfer.go` L140). Target path is empty; old content is irreversibly lost |
...
| Standard (0) — error | +1 (new) or 0 (overwrite)† | +partial bytes | Yes | Partial bytes counted |
| Atomic (1) — success | +1 (new) or 0 (overwrite)† | +actual bytes | Yes | Normal update |
| Atomic (1) — error, new file, temp deleted OK | 0 | 0 | **No** (line 166: `0 != 0 \|\| 0 > 0` → false) | **Clean reversal** — no quota change |
| Atomic (1) — error, overwrite, temp deleted OK | **-1** | 0 | **Yes** (`-1 != 0` → true) → `UpdateUserQuota(-1, 0)` | File count decremented by 1; combined with prior -fileSize deduction†, correctly reflects both files gone |
| Atomic (1) — error, temp delete fails | +1 (new) or 0 (overwrite)† | +partial bytes | Yes | **Temp file leaked + quota inconsistency** |
| Atomic+resume (2) — error | +1 (new) or 0 (overwrite)† | +partial bytes | Yes | Intentional for resume support |

> **† Overwrite quota footnote**: For **all** overwrite scenarios (regardless of upload mode), an additional `UpdateUserQuota(0, -fileSize, false)` call occurs **at upload initiation** — before any data transfer begins — to deduct the old file's size from the quota. This happens at `sftpd/handler.go`, line 486 (SFTP) and `sftpd/scp.go`, line 282 (SCP). The Δ values in the table above represent **only** the contribution from `Transfer.Close()`. To reconstruct the full quota accounting for an overwrite, sum the initiation-time deduction (`0 files, -oldFileSize bytes`) with the close-time delta shown in the table.
