...
4. **The `/metrics` `sftpgo_http_*` counters are header-independent.** `HTTPRequestServed(status)`
   ...
   very same `Write()` that emits the access log `[logger/request_logger.go:57-58]`. Repeating an
...
`[sftpgo.json:24-51]`. It does, however, differ in at least one value — `track_quota` is `2` in the
...
`[middleware/realip.go:40-53]`. Mind the `", "` split quirk: a comma-without-space `X-Forwarded-For`
