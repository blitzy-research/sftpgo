...
> **Repository:** `drakkan/sftpgo` · **Version:** `0.9.5-dev` · **HEAD:** `44634210287cb192f2a53147eafb84a33a96826b`
...
- **Stability.** Every matrix cell was executed **twice** and confirmed byte-identical for both `argv` and credentials. `STABLE_ACROSS_2_RUNS = True` for all cells (51 spawn captures in total).

---

## 3. The Code Path That Builds the argv (exact producing functions)

The dispatch chain from the SSH channel request to the `execve()` boundary is an ordered walk through the following exact functions. Every step is cited to `file:line` at HEAD `44634210287c`.
...
The `argv` **shapes** are identical to the mapped rows above, but the home directory is `/tmp/inv/home/u_nomap_sym/…` and the **child credentials are `uid=0`** — because `GetUID()` returned `-1` (`dataprovider/user.go:237-242`), so `wrapCmd`'s guard `uid > 0 || gid > 0` was false (`sftpd/cmd_unix.go:11`) and the child **inherited the server's own privileges**. Representative captures:

| Client-sent command | Observed argv at `execve()` | Child creds |
|---|---|---|
| `git-upload-pack /incoming` | `[git-upload-pack, /tmp/inv/home/u_nomap_sym/incoming]` | `uid=0` |
| `rsync --server --sender -e/bin/sh . ../../../../etc/passwd` | `[rsync, --safe-links, --server, --sender, -e/bin/sh, ., /tmp/inv/home/u_nomap_sym/etc/passwd]` | `uid=0` |

The remaining commands mirror the ON rows in shape (same `argv`, same normalization, same `--safe-links`), differing only in home dir and in the `uid=0` credentials. **This ON-vs-OFF difference — `uid=1000` when mapped versus `uid=0` when unmapped, for otherwise identical `argv` — is the direct privilege-boundary datum** and is analyzed in Section 7.
...
- `git status --porcelain` against the baseline is **EMPTY** for all tracked source files; **HEAD remains `44634210287cb192f2a53147eafb84a33a96826b`**; `go.mod` and `go.sum` are **unchanged**.
- All investigation scaffolding lived **entirely outside the repository tree** — under `/tmp/inv` (config, home dirs, SQLite DB, logs, observer shims, SSH driver) and `/tmp/sftpgo_bin` (the built binary) — and was removed on completion. No temporary file was ever written inside the repository.
