...
When no config file is found (or loading fails), the defaults from `config/config.go:init()` (lines 41–102) are used:
...
```json
{"level":"debug","time":"<TIMESTAMP>","sender":"sqlite","connection_id":"","message":"sqlite database handle created, connection string: \"file:<CONFIG_DIR>/sftpgo.db?cache=shared\""}
```
