...
`zerolog` JSON records emitted to the log file; protocol bytes are shown as `od` hex;
...
- **OpenSSH 10 `scp` uses the SFTP protocol by default.** A plain `scp` therefore
  ...
  `sftpd/scp.go`. This is demonstrated in §3.2 and §4.
- **`rsync` here is 3.4.1**, whose hardened `--server` handshake rejects the option
  that SFTPGo 0.9.5‑dev injects; this is shown and explained in §3.5 / §6.4.
...
**Actual execution outcome (honest, [observed]).** Whether the process can even run
depends on the UID/GID model (§3.9). With `test` at the API‑default `uid=0 gid=0`
the process *does* fork and run as the server user, and the server relays the
streams, but the **rsync transfer itself exits 12** — because the `--safe-links`
option SFTPGo 0.9.5‑dev prepends *before* `--server` is rejected by rsync 3.4.1's
hardened server‑arg check:
...
So under this modern toolchain `rsync` is **accepted, sanitized, and dispatched**, and
the security‑relevant behavior (allow‑listing, last‑arg path resolution, symlink‑flag
injection) is fully observable; the transfer's non‑zero exit is a 2020‑SFTPGo ↔
2024‑rsync version interaction, not an authorization outcome.
...
`rsync: /tmp/sftpgo_obs/home/dest.txt Permission denied. You don't have the permissions to execute this command`
— onto the channel where `rsync` expects its protocol handshake, so `rsync` reports
`protocol version mismatch` and exits 2.
...
require a **server restart**, not merely a reconnect — as used between §4.3 and §4.4.)
...
if len(parts) == 3 {                           // exactly three (:643)
    ...
    name = parts[2]                                  // field 2 = name (:653, non-empty guard follows)
} else {
...
51‑byte reply captured above. This is the same class of single‑space assumption as the
...
was exercised, and one has a demonstrable **gap** and one a demonstrable **race**:
...
- **Symlink escapes are caught** by `ResolvePath` resolving symlinks with
  `filepath.EvalSymlinks` (`vfs/osfs.go:205`) and then calling `isSubDir`
  (`vfs/osfs.go:218`) to confirm the resolved target is inside home.
...
(`vfs/osfs.go:222`) and re‑resolving it at use time. The win rate is low and
non‑deterministic (≈0.25–0.5 % here), but the escape is **real and observed**, reproduced
in both runs.
...
| symlink → outside (non‑prefix) | `EvalSymlinks`+`isSubDir` (`vfs/osfs.go:205,218`) | **Blocked** | E5.2, E5.3b |
...
attempts (it was only ever read, in the two TOCTOU wins, never modified).
...
- **Observed** — the behavior was triggered at runtime against the canonical server and its
  raw output (client stdout/stderr + exit code, raw zerolog JSON from `sftpgo.log`, and/or
...
| Executable reproducibility appendix | Observed | §11 — ordered setup/run/reset/teardown + harness source |
...
With the server running (§11.4), provision a user with a real, captured API response. The
request body and the **actual HTTP 200 response** from this run were:
...
The canonical `test` user (id 1) used by the experiments has `home_dir=/tmp/sftpgo_obs/home`,
`uid=0, gid=0`, and baseline `permissions={"/":["*"]}`; individual experiments mutate its
permissions via `PUT /api/v1/user/1` and restore the baseline afterward (§11.5).
...
BIN=/tmp/sftpgo_obs/harness/sftpcli_bin
run_cap() {
  local name="$1"; shift
...
cd /tmp/blitzy/sftpgo/blitzy-7c33126a-2c9d-4a37-a9a7-38b63238492b_0390dc
git status --porcelain      # -> only: M blitzy/documentation/sftpgo_44634210287c.md
```
