...
`Source: dataprovider/sqlqueries.go:43-50`
...
The following state diagram shows the full lifecycle of a single SFTP connection from TCP accept through removal:

```mermaid
stateDiagram-v2
    [*] --> TCPAccept: listener.Accept()
    TCPAccept --> SSHHandshake: go AcceptInboundConnection()
    SSHHandshake --> AuthCallback: ssh.NewServerConn()
    AuthCallback --> AuthFailed: loginUser fails\n(bad creds / MaxSessions / IP filter)
    AuthCallback --> AuthSuccess: loginUser passes\nPermissions returned
    AuthFailed --> [*]: ConnectionFailedLog emitted
    AuthSuccess --> ChannelAccept: ssh.NewChannel negotiated
    ChannelAccept --> SFTPSubsystem: req.Type == "subsystem"\npayload == "sftp"
    SFTPSubsystem --> Registered: addConnection()\nmutex.Lock()
    Registered --> TransferActive: Filewrite/Fileread request\naddTransfer()
    Registered --> IdleCheck: CheckIdleConnections\nidleTime > idleTimeout
    TransferActive --> TransferActive: WriteAt/ReadAt\nlastActivity updated
    TransferActive --> TransferComplete: Transfer.Close()\nremoveTransfer()
    TransferComplete --> Registered: Ready for next request
    TransferActive --> IdleCheck: No I/O activity\nidleTime > idleTimeout
    IdleCheck --> ConnectionClose: c.close()\nchannel.Close() + netConn.Close()
    Registered --> ConnectionClose: Client disconnects
    ConnectionClose --> Removed: defer removeConnection()\nmutex.Lock()
    Removed --> [*]: Connection lifecycle complete
```

`Source: sftpd/server.go:192-353` (TCP accept through SFTP subsystem), `Source: sftpd/sftpd.go:354-376` (addConnection/removeConnection), `Source: sftpd/sftpd.go:331-352` (CheckIdleConnections), `Source: sftpd/handler.go:536-542` (Connection.close)
...
`Source: dataprovider/sqlcommon.go:74-90`, `Source: dataprovider/sqlqueries.go:43-50`
...
#### Activity Resolution Decision Flowchart

The following flowchart details the per-connection idle-time resolution logic inside `CheckIdleConnections`, showing how transfer timestamps are compared against the connection's idle time:

```mermaid
flowchart TD
    A["For each connection c in openConnections"] --> B["idleTime = time.Since(c.lastActivity)"]
    B --> C["For each transfer t in activeTransfers"]
    C --> D{"t.connectionID == c.ID?"}
    D -->|No| C
    D -->|Yes| E["transferIdleTime = time.Since(t.lastActivity)"]
    E --> F{"transferIdleTime < idleTime?"}
    F -->|Yes| G["idleTime = transferIdleTime<br/>(use more recent activity)"]
    G --> C
    F -->|No| H["Keep current idleTime<br/>(connection activity is more recent)"]
    H --> C
    C -->|All transfers checked| I{"idleTime > idleTimeout?"}
    I -->|Yes| J["c.close()<br/>Log: 'close idle connection, idle time: ...'"]
    I -->|No| K["Connection stays open"]
```

`Source: sftpd/sftpd.go:331-352` — The `transferIdleTime < idleTime` comparison at line 339 is the critical decision node that prevents closing connections with active transfers.
...
`Source: logger/logger.go:84-116` (Log/Debug/Info/Warn/Error), `Source: logger/logger.go:138-150` (TransferLog), `Source: logger/logger.go:152-169` (CommandLog), `Source: logger/logger.go:175-184` (ConnectionFailedLog)
