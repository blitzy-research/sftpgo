...
There is no `3xx` branch — so a redirect bumps **only** `req_total`.

One subtlety makes the *absolute* counter values worth pinning down precisely: **reading `/metrics`
is itself an HTTP request that this very middleware counts** — but the increment is *deferred*. chi's
`RequestLogger` runs `entry.Write(...)` — the method that calls `metrics.HTTPRequestServed(status)` —
inside a `defer` that fires only **after** the handler has returned
`[middleware/logger.go:42-46; logger/request_logger.go:57-58]`. Because `promhttp` gathers the counter
values **while** that handler runs (i.e. *before* the deferred increment), **a `/metrics` read never
counts itself in its own body** — it is counted only by the *next* read. So any absolute `/metrics`
snapshot reads as "every request that completed *before* this read."

With that in mind: on a freshly started server, after exactly the E3 probe sequence (`GET /`, `/web`,
`/metrics`, `/nope`, `/api/v1/missing`, and one more 404 — six requests, the third of which is the
in-sequence `/metrics`), the **first** `/metrics` read returns a body reflecting only those six
requests:
...
`req_total` but **not** `req_ok` — confirming the missing `3xx` branch. This snapshot does **not**
include the snapshot read itself; that request's increment is still pending in the deferred `Write()`.

Read `/metrics` a **second** time and the counters advance by exactly one `200` — because the first
read is now counted:

```text
sftpgo_http_req_total 7
sftpgo_http_req_ok_total 2
sftpgo_http_client_errors_total 3
sftpgo_http_server_errors_total 0
```

Only `req_total` and `req_ok` move (by that one self-referential `/metrics` read); `client_errors`
and `server_errors` are unchanged. This is exactly why the header-independence proof below is stated
as **deltas** rather than absolutes: a delta brackets each round between two reads, cancelling the
self-counting so the result is reproducible no matter how many times `/metrics` has already been
polled.
