...
The last argument `/testrepo` was jailed to `/tmp/homeA/testrepo` exactly as for `rsync`, but **no `--safe-links`/`--munge-links`** was added — that injection is gated on `c.command == "rsync"` (`sftpd/ssh_cmd.go:L306-L306`).
...
```mermaid
flowchart TD
    A["SSH exec request: command string"] --> B["processSSHCommand\nunmarshal sshSubsystemExecMsg (sftpd/ssh_cmd.go:L45-L45)"]
