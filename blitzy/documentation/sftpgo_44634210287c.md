...
The generated `/tmp/sftpgo_probe/cfg/sftpgo.json` used for every capture is the repository sample with exactly those deviations applied. The one change that makes the exec path reachable — the `enabled_ssh_commands` array — is shown here as the literal JSON written (the daemon confirms it on load: `EnabledSSHCommands:[md5sum sha1sum cd pwd rsync git-upload-pack git-receive-pack git-upload-archive]`, `daemon.log`):
...
```json
{
  ...
  "data_provider": { "driver": "sqlite", "name": "/tmp/sftpgo_probe/sftpgo.db", "track_quota": 2 },
  "httpd": { "bind_port": 8080, "bind_address": "127.0.0.1" }
}
```

(Fields not shown are inherited from the repository sample `sftpgo.json` unchanged; `httpd.templates_path`/`static_files_path` point at the checkout's `templates/` and `static/` directories so the management API can serve, without copying anything into the checkout.)
...
# The API echoes each created record with "uid":0,"gid":0. All three users keep the default
# record uid:0,gid:0 (unmapped) — verified directly in SQLite:
#   $ sqlite3 /tmp/sftpgo_probe/sftpgo.db 'SELECT username,uid,gid FROM users ORDER BY username;'
#   usera|0|0
#   userb|0|0
#   userc|0|0
# The ONLY difference between usera and userb is the create_symlinks permission — which is exactly
# what flips --safe-links vs --munge-links. The mapped positive-uid credential branch of wrapCmd is
# deliberately NOT exercised at runtime (running a mapped uid/gid is out of scope); it is code-derived
# (INFERRED) in Section 5 from wrapCmd + GetUID/GetGID.
```
...
$ export SHIM_TAG=CELL1   # optional coarse run-label; the shim (the daemon's child) inherits it
...
Each cell's captured `argv` is bound to its record deterministically by the before/after line count of `$SHIM_LOG` — **not** by the `tag` field: the shim is the daemon's child and inherits the *daemon's* fixed environment, so a client-side `SHIM_TAG` never reaches it and `SHIM_TAG` (if used at all) is set once on the daemon at launch (§1.5(d)) as a coarse run-label. Because the shim's environment is fixed for the whole run, its records are strictly ordered and map one-to-one to the invocations in sequence. Every cell was run in **three passes** for stability (Section 2).
...
The matrix crosses **normal vs. adversarial** invocation with **`--safe-links` vs. `--munge-links`** permission context (User A = has `create_symlinks`; User B = does not). For every cell below, three independent views are shown and **agree**: **[shim]** the child's own `os.Args`; **[daemon]** SFTPGo's build-line at `sftpd/ssh_cmd.go:323` (sender `"ssh"`); **[strace]** the kernel `execve(2)`. Every `argv`, `exe`, `uid`, and `gid` value shown is real captured output, quoted verbatim (pass 1 shown; passes 2–3 are byte-identical for these fields). To keep each cell readable, the `[shim]` line prints the record's **stable core fields** (`exe`, `argv0`, `argv`, `argc`, `cwd`, `uid`/`euid`/`gid`/`egid`) and elides three **volatile** fields the shim also records — `ts` (a wall-clock instant), `ppid` (the daemon's PID), and `tag` (a coarse run-label the shim reads from its inherited `SHIM_TAG`) — which vary run-to-run and are therefore **deliberately excluded from the stability hash** (see "argv stability & the exact hash recipe"). So there is no hidden editing, one **complete, unedited 12-field record** is reproduced in full immediately below CELL 1 to document the entire schema.
...
**Complete, unedited shim record (all 12 fields — CELL 1, one representative run).** The per-cell `[shim]` lines throughout Section 2 elide the volatile `ts`/`ppid`/`tag`; here is one whole record exactly as the shim wrote it to `$SHIM_LOG`, to document the full schema. This run set `SHIM_TAG=CELL1` when launching the daemon (see §1.5(d)); the shim — the daemon's child — inherits it as its own environment, which is why `tag` is populated here:

```text
{"ts":"2026-07-13T22:00:00.110338722Z","exe":"/tmp/sftpgo_probe/shimbin/rsync","argv0":"rsync","argv":["rsync","--safe-links","--server","-vlogDtprze.iLsfxC",".","/tmp/sftpgo_probe/homes/usera/data"],"argc":6,"cwd":"/tmp/sftpgo_probe","uid":0,"euid":0,"gid":0,"egid":0,"ppid":131364,"tag":"CELL1"}
```

The `argv`/`exe`/`uid`/`gid` in this full record are byte-identical to the CELL 1 `[shim]` line above, so its stability hash is unchanged (`94451e43…`); only the volatile `ts`, `ppid` (`131364` on this run — a *different* daemon run than the `65039` shown in the per-cell lines), and `tag` differ. `argc` (`6`) equals `len(argv)`.
...
[daemon built] {"sender":"ssh","message":"new system command \"git-upload-pack\", with args: [/tmp/sftpgo_probe/homes/usera/repo.git] path: /tmp/sftpgo_probe/homes/usera/repo.git"}
[shim ]  {"exe":"/tmp/sftpgo_probe/shimbin/git-upload-pack","argv0":"git-upload-pack","argv":["git-upload-pack","/tmp/sftpgo_probe/homes/usera/repo.git"],"argc":2}
[strace]  execve("/tmp/sftpgo_probe/shimbin/git-upload-pack", ["git-upload-pack", "/tmp/sftpgo_probe/homes/usera/repo.git"], 0xc000202000 /* 87 vars */) = 0
```
...
For each launching case, a sha256 over the record's four **stable** fields `{argv, uid, gid, exe}` is **identical across all three passes**; the two non-launching edge cases produce zero OS processes on all three passes (expected).
...
```python
sha256(json.dumps({"argv": rec["argv"], "exe": rec["exe"],
                    "gid": rec["gid"], "uid": rec["uid"]},
...
which hashes to `94451e430e…`. The volatile fields (`ts`, `ppid`, `tag`, and the `execve` environment-array pointer) are intentionally **not** part of the hashed object (see the nondeterminism note below), which is exactly why the digests reproduce across independent daemon runs. Full manifest (`stability_manifest.json`):
...
**Nondeterminism note (what is *not* stable, and why it is excluded from the hash).** Three fields the shim/`strace` also record **do** vary run-to-run and are deliberately omitted from the digest: (1) `ppid` — the daemon's PID; (2) the `execve(2)` **environment-array pointer** (`0xc0…`), a Go-runtime heap address; and (3) `ts` — a wall-clock instant. Repeating CELL 1 across three fully independent daemon launches makes this concrete — the hashed fields are invariant while these volatile values all change:

| Independent run | `ppid` (daemon PID) | `execve` env-array pointer | env var count | sha256`{argv,exe,uid,gid}` |
|-----------------|---------------------|----------------------------|---------------|-----------------------------|
| run A | `65039` | `0xc000362300` | 89 | `94451e430e…` |
| run B | `129881` | `0xc000202000` | 87 | `94451e430e…` (identical) |
| run C | `131364` | `0xc000354300` | 88 | `94451e430e…` (identical) |

The environment-array pointer is a heap address the Go runtime hands to `execve`; it is **constant within a single daemon run** (every child exec in run B shared `0xc000202000`) but **differs across runs**, and the env var *count* is environment-dependent (89/87/88 above). The daemon's own top-level `execve` instead shows a *stack* pointer (`0x7fff…`). None of these touch the client-controlled `argv`, which is why the security-relevant conclusions (S1/S2/S3) and every cell digest are stable regardless.
...
[daemon built] {"sender":"ssh","message":"new system command \"git-upload-pack\", with args: [] path: "}
[shim ]  {"exe":"/tmp/sftpgo_probe/shimbin/git-upload-pack","argv0":"git-upload-pack","argv":["git-upload-pack"],"argc":1,"uid":0,"euid":0,"gid":0,"egid":0}
[strace]  execve("/tmp/sftpgo_probe/shimbin/git-upload-pack", ["git-upload-pack"], 0xc000202000 /* 87 vars */) = 0
```
`parseCommandPayload` returns empty args for a single token (`sftpd/ssh_cmd.go:425-427`), so `getSystemCommand` skips path resolution (the `len(c.args) > 0` guard at `:296`); `argv` is just the bare command. `argc == 1 == len(argv)`. sha256=`e3a869dc…`.
...
[daemon built] {"sender":"ssh","message":"new system command \"rsync\", with args: [--safe-links] path: "}
[shim ]  {"exe":"/tmp/sftpgo_probe/shimbin/rsync","argv0":"rsync","argv":["rsync","--safe-links"],"argc":2,"uid":0,"euid":0,"gid":0,"egid":0}
[strace]  execve("/tmp/sftpgo_probe/shimbin/rsync", ["rsync", "--safe-links"], 0xc000202000 /* 87 vars */) = 0
```
Even with **no** client arguments, the `if c.command == "rsync"` block at `sftpd/ssh_cmd.go:306` still runs and **prepends `--safe-links`** (User A has `create_symlinks`), so `argv` is `["rsync", "--safe-links"]` — **argc 2**, not 1. Note the `path:` is empty: the empty-args guard at `:296` skips destination resolution, yet the safety-flag insertion at `:306-322` is *independent* of that guard and executes anyway. This isolates the two mechanisms: path resolution is gated on non-empty args; the safety flag is gated only on `command == "rsync"` and the `create_symlinks` permission. `argc == 2 == len(argv)`. sha256=`e69e5c4528d885b0dca63f82aaaf3b433cf4e782a2056a3d4af3e097e9ea6685`.
...
**Verdict:** the flags are **not** cosmetic (they function), but the surrounding **argument policy is thin and client-influenceable** — so the suspicion is *half right*. This **MIXED** result is a deliberate, evidence-grounded finding, not indecision: each half is settled by a *different* runtime observation — the real-`/usr/bin/rsync` run refutes the "cosmetic no-op" half, while the `argv` captures confirm the "not a real policy" half — so collapsing it to a single CONFIRMED/REFUTED label would misstate whichever half it dropped. **Scope/limitation (explicit):** the 2×2 shim captures record `argv` only; the shim performs no file I/O, so this investigation did **not** construct a working end-to-end *bypass of an active `--safe-links`* for a `create_symlinks` user. The `argv`-layer weaknesses (verbatim option pass-through, no allow-list, forced flag contradiction) are proven by capture; the downstream real-rsync run shows the flags are honored when present.
