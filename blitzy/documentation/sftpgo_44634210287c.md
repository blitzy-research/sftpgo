...
# userc  -> like userb + uid:1000,gid:1000  (exercises wrapCmd credential; Section 5)
# userd  -> like userb but MISSING "delete"  (exercises the permission-denied gate; Section 2 edges)
```
...
| CRED_C | `be783611d7fa6e7ce6ecb21f13b8f82c7080074c40fb7207f54a8688646df0e7` | YES ✓ |
...
**EDGE_PERMDENIED — construction happens, launch does NOT (User D, missing `delete`):** `rsync --server . data`
```text
[daemon build] {"level":"debug","sender":"ssh","message":"new system command \"rsync\", with args: [--munge-links --server . /tmp/sftpgo_probe/homes/userd/data] path: /tmp/sftpgo_probe/homes/userd/data"}
[daemon fail ] {"level":"warn","sender":"ssh","message":"command failed: \"rsync\" args: [--server . data] user: userd err: Permission denied. You don't have the permissions to execute this command"}
[shim ]  (zero records — no OS process launched)
```
...
The credential is applied **only if `uid > 0 || gid > 0`**, using the *user record's* mapped uid/gid from `User.GetUID()`/`GetGID()` (`dataprovider/user.go:237,245`; `GetUID` returns `-1` when the record's uid is `≤0` or `>65535`). This is confirmed at runtime:

```text
# userc has uid:1000,gid:1000 in its record -> wrapCmd APPLIES the credential:
[shim CRED_C] argv=["rsync","--munge-links","--server",".","/tmp/sftpgo_probe/homes/userc/data"]
              uid:1000, euid:1000, gid:1000, egid:1000     <-- child dropped to 1000:1000

# usera/userb/userd are unmapped (record uid 0 -> GetUID returns -1) -> wrapCmd does nothing:
[shim CELL1] uid:0, euid:0, gid:0, egid:0                  <-- child inherits the daemon's uid
```

So the child's privilege is a **property of the daemon's own uid and the user's mapping, not of the argv.** In this run the daemon happened to run as root, so unmapped users' children ran as root (`uid:0`) — but that is the *deployment's* choice of who runs SFTPGo, not something the client-controlled argv caused or could change. A mapped user (userc) was correctly confined to `1000:1000`.

**Precise boundary statement.** The captures prove a **containment/sanitization concern** (client-shaped argv + thin rsync-argument policy), which becomes a **privilege concern only in deployments that (a) run SFTPGo as a high-privilege account and (b) leave users unmapped** — because then the client-shaped argv executes with that high privilege. The evidence does **not** demonstrate a *privilege-escalation defect in SFTPGo itself*: there is no path where the argv rewrote its own credentials, and a mapped user was correctly dropped. **Scope caveat:** the credential observation is on Linux (`cmd_unix.go`; `// +build !windows`); on Windows `wrapCmd` is a no-op (`sftpd/cmd_windows.go:7-9`), so no credential is ever applied there. Two further honesty caveats: the shim runs as whatever `wrapCmd` set but performs **no** privileged action, so this is evidence of the *credential the process would carry*, not of a completed privileged operation; and the daemon here ran as root, so the elevated-child observation is scoped to that (common) deployment.
...
| Privilege-boundary implication | Containment concern (argv client-shaped) is real; privilege concern only for root+unmapped deployments; no SFTPGo self-escalation shown; mapped user dropped to 1000:1000 | Section 5 |
...
**Observed vs. inferred.** Every behavioral claim above is *observed* (captured argv, logs, strace, real-rsync results, credential uids). The only *inferred* statements are the internal mechanics of `exec.Command` (that it sets `Args[0]` to the name and resolves `Path` via `PATH`) — inferred from Go's documented `os/exec` contract but independently corroborated by the shim's `argv0` vs `exe` split.
