...
_Source: `cmd/root.go:69-73`_ — `Execute()` simply calls `rootCmd.Execute()`, which dispatches to the appropriate subcommand.
...
_Source: `service/service.go:69-73`_
...
_Source: `httpd/httpd.go:75-98`_
...
{"level":"info","time":"2024-01-15T10:30:00.004","sender":"sftpd","connection_id":"","message":"No host keys configured and \"./id_rsa\" does not exist; creating new private key for server"}
...
```json
{
  ...
  "sender": "sqlite",
  "connection_id": "",
  "message": "error authenticating user: ghost_user, error: sql: no rows in result set"
}
```
...
After all authentication attempts are exhausted, the SSH library returns a `*ssh.ServerAuthError`. The server logs this at `sftpd/server.go:251`:
...
```json
{
  ...
  "sender": "sftpd",
  "connection_id": "",
  "message": "failed to accept an incoming connection: ssh: disconnect, reason 2: too many authentication failures"
}
```
...
From the client's perspective, the SSH server sends `SSH_MSG_USERAUTH_FAILURE` after each failed attempt. After exhausting `MaxAuthTries` (default: 6 internal to the Go SSH library), the connection is terminated. A typical `ssh` client would display:
...
```go
// service/service.go lines 69-73
err := dataprovider.Initialize(providerConf, s.ConfigDir)
if err != nil {
    logger.Error(logSender, "", "error initializing data provider: %v", err)
    logger.ErrorToConsole("error initializing data provider: %v", err)
...
| `cmd/root.go` | CLI flags and defaults (lines 15-41), `Execute()` (lines 69-73), Viper env bindings (lines 76-133) |
| `cmd/serve.go` | Serve command wiring (lines 17-33), `Service` struct construction |
| `config/config.go` | Default config struct `init()` (lines 41-101), `LoadConfig()` (lines 145-182), `getRedactedGlobalConf()` (lines 134-138), banner construction (line 32) |
