...
- **System tools installed for the OS-process path** (absent in the base image): `rsync  version 3.2.7  protocol version 31` and OpenSSH client `OpenSSH_9.6p1 Ubuntu-3ubuntu13.16, OpenSSL 3.0.13 30 Jan 2024`. The git `*-pack` binaries were already on `PATH`.
...
- **rsync `--munge-links`** — Ubuntu focal rsync(1) manpage: it makes symlinks "unusable but recoverable"; man7.org rsync(1): "The way rsync disables the use of symlinks is to prefix each one with the string '/rsyncd-munged/'." — a genuine, symlink-scoped mitigation (corroborates Suspicion B being real but narrow).
