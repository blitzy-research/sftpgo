...
**Blocking mechanism:** `touch` is a common Unix utility but is emphatically **not** in `supportedSSHCommands`. This test confirms that the whitelist is effective even with `enabled_ssh_commands: ["*"]`, because `"*"` expands only to the contents of `supportedSSHCommands` (see `sftpd/sftpd.go:143-145`, `GetSupportedSSHCommands()`, which is used to substitute the wildcard). The set `supportedSSHCommands` is therefore the true upper bound on reachable commands. **File `/tmp/audit_touch.txt` was NOT created.**
...
`c.command` is the **command name only**, set at `sftpd/ssh_cmd.go:71`:
...
This is the maximum command set — the union of all commands the server can ever accept. When `enabled_ssh_commands: ["*"]` is configured, the wildcard is expanded to the full `supportedSSHCommands` slice via `GetSupportedSSHCommands()` (`sftpd/sftpd.go:143-145`). This means an attacker cannot inject a new command name via the config — `"*"` gives them exactly these 12 names and nothing else. The 12 names are:
...
- `sql/sqlite/001_init.sql` — Initial user table schema for SQLite provider.
