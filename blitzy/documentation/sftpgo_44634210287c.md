...
**Source**: `dataprovider/user.go:237-249`
...
**Windows**: `sftpd/cmd_windows.go:7-8` — `wrapCmd` is a no-op that returns `cmd` unchanged. No credential isolation exists on Windows.
...
**Path resolution step-by-step** (for adversarial input `/data/../../../etc/passwd` with homeDir `/tmp/secaudit_test_home`):

1. `filepath.Join("/tmp/secaudit_test_home", "/data/../../../etc/passwd")` → `"/tmp/secaudit_test_home/data/../../../etc/passwd"`
2. `filepath.Clean(...)` → `"/tmp/secaudit_test_home/etc/passwd"` (traversal sequences `../../` are collapsed — `filepath.Clean` applies `..` resolution AFTER joining with `rootDir`)
3. `filepath.EvalSymlinks(...)` → resolves any symlinks to real targets
4. `isSubDir(resolved, rootDir)` → validates `strings.HasPrefix("/tmp/secaudit_test_home/etc/passwd", "/tmp/secaudit_test_home")` → `true` → **PASSES**

**Traversal blocked**: The critical insight is that `filepath.Clean(filepath.Join(rootDir, sftpPath))` collapses `..` sequences **relative to the joined path**. The input `/data/../../../etc/passwd` becomes `/tmp/secaudit_test_home/etc/passwd` (confined to home), NOT `/etc/passwd`.
...
**Location**: `sftpd/cmd_unix.go:10-16`, `dataprovider/user.go:237-249`
...
SHA-256 checksums of all 139 tracked repository files were computed before and after the audit evidence gathering.
...
**Repository confirmed exactly unchanged.** All 139 tracked files are bit-identical before and after the audit.
...
Repository integrity verified unchanged via SHA-256 checksum comparison of all 139 tracked files.
